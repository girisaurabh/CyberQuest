  ...
  return (
    <AuthLayout eyebrow="START YOUR JOURNEY" title="Create your CyberQuest" subtitle="Build a real cybersecurity learning profile and track your progress over time.">
      <form onSubmit={handleSubmit} className="space-y-4">
        {[
          ...
          <label key={key} className="block">
            ...
            <input required minLength={key === "password" ? 8 : undefined} type={type} value={form[key]} onChange={(e) => setForm({ ...form, [key]: e.target.value })} className="w-full rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3.5 text-white outline-none focus:border-cyan-400/50" />
          </label>
        ))}
      ...
      <p className="mt-6 text-center text-sm text-slate-500">
        Already have an account? <Link className="text-cyan-400 hover:text-cyan-300" to="/login">Sign in</Link>
      </p>
    </AuthLayout>
  );
}
