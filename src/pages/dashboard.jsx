...
function Dashboard() {
  ...
  async function completeMission() {
    ...
    setMissionBusy(true);
    try { await api.completeMission(data.todayMission.id); await loadDashboard(); }
    catch (err) { setError(err.message); }
  ...
  return <div className="min-h-screen">
    ...
    <main className="p-6 md:p-10 max-w-[1500px] mx-auto">
      <section className="mb-8"><p className="text-blue-600 text-sm font-medium mb-2">KEEP BUILDING 🚀</p><h1 className="text-4xl md:text-5xl font-bold tracking-tight text-slate-900">Good to see you, <span className="text-blue-600">{data.user.name}</span> 👋</h1><p className="text-slate-500 mt-3 text-lg">Build real cybersecurity skills, one mission at a time.</p>{error && <p className="mt-4 text-sm text-red-600">{error}</p>}</section>
